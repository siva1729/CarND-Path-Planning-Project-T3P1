      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
			...
			// Predictions:
			// Calculate/Predict the Presence of Vehicles in lanes, Max speed, Buffer Distance,
            // Average Lane velocities based on SensorFusion Data for each lane

			vector<bool> cars_in_lanes = {false, false, false}; // {Left, Ahead, Right}
			vector<double> avgspeed_in_lanes = {0, 0, 0}; // {Left, Ahead, Right}
            vector<double> maxspeed_in_lanes = {-1, -1, -1}; // {Left, Ahead, Right}
			vector<int> count_in_lanes = {0, 0, 0}; // {Left, Ahead, Right}
			vector<double> buffer_in_lanes = {9999, 9999, 9999}; // {Left, Ahead, Right}
            vector<double> backbuf_in_lanes = {9999, 9999, 9999}; // {Left, Ahead, Right}

            const double BACK_BUF_DIST = 30;
            const double FRONT_BUF_DIST = 30;

			double back_buf_dist = BACK_BUF_DIST;
			double front_buf_dist = FRONT_BUF_DIST;
			...
			for (int lane = LL; lane <= RL; lane++)
			{
				if (count_in_lanes[lane] == -1)     // Invalid lane
					avgspeed_in_lanes[lane] = -1;     // small number
				else if (count_in_lanes[lane] == 0) // No cars in the lane
					avgspeed_in_lanes[lane] = max_vel;
				else
					avgspeed_in_lanes[lane] /= ((double)count_in_lanes[lane]); //average speed
			}
			...
			int ASLL = avgspeed_in_lanes[0];
			int ASGL = avgspeed_in_lanes[1];
			int ASRL = avgspeed_in_lanes[2];

            int MSLL = maxspeed_in_lanes[0];
			int MSGL = maxspeed_in_lanes[1];
			int MSRL = maxspeed_in_lanes[2];
            ...
            int BBLL = (backbuf_in_lanes[0] == 9999) ? 0 : backbuf_in_lanes[0];
            int BBGL = (backbuf_in_lanes[1] == 9999) ? 0 : backbuf_in_lanes[1];
            int BBRL = (backbuf_in_lanes[2] == 9999) ? 0 : backbuf_in_lanes[2];
			//int BBGL = backbuf_in_lanes[1];
			//int BBRL = backbuf_in_lanes[2];
